...
require("dotenv").config(); // env 파일을 불러와서 실행.
...
const iterations = 100; //반복 회수
const keylen = 64; //생성할 키의 길이
const digest = process.env.HASH2; //해시 알고리즘

//단방향 암호화 생성
const createPbkdf = (password) => {
    //pbkdf2함수(비밀번호, 솔트, 반복횟수, 키의길이, 알고리즘)
    //으로 생성이 되고, 반환되는 값은 Buffer값
...
const iv = crypto.randomBytes(16); //초기화 벡터, 데이터 블록을 암호화할때 보완성을 높이기 위해 사용.
// initialize vector 약자 ->> iv
